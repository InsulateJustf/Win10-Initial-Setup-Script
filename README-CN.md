## 目录
 ...
 - [进阶使用](#advanced-usage)
 - [维护自己的分支](#maintaining-own-forks)
 - [代码贡献指南](#contribution-guidelines)
...
这个脚本支持命令行选项和参数，这些选项和参数可以帮助您自定义您的调整选择。甚至您还可以添加您自定义的调整，但是这些功能需要一些命令行和 Powershell 脚本编写的基础知识。更多细节请参考[进阶使用](#进阶使用)部分。
...
这个调整库由独立的幂等函数组成，每个函数包含了一个调整。预设 *presets* 中包含了这些函数。预设只是一个被调用的函数名列表。任何在预设中没有出现或者被注释的函数都不会被调用，因此相应的调整也不会生效。你需要通过 `-include` 或者通过 `-preset` 提供一个调整库和至少一个调整名，或者直接作为命令行参数
